...
public class EntityHook extends Entity implements IProjectile
{
    ...
    public void onUpdate()
    {
		...
		if(!this.worldObj.isRemote&&(shootingEntity.getCurrentItemOrArmor(2) == null||shootingEntity.getCurrentItemOrArmor(2).getItem() != ssTanksMOD.インスタンス.moveleg)){
			if(ssTanksMOD.インスタンス.moveleg.LeftHook.containsKey(shootingEntity)){
				this.setDead();
				ssTanksMOD.インスタンス.moveleg.LeftHook.remove(shootingEntity);
			}
			if(ssTanksMOD.インスタンス.moveleg.RightHook.containsKey(shootingEntity)){
				this.setDead();
				ssTanksMOD.インスタンス.moveleg.RightHook.remove(shootingEntity);
			}
		}
